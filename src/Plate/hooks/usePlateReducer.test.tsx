...
describe("usePlateReducer layers", () => {
  test("creates, activates, renames, moves, and deletes layers", () => {
    ...
    const { result } = renderHook(() =>
      usePlateReducer({ initialPlateSize: 96, generateId: () => ids[idIndex++] }),
    );
    expect(result.current.plateState.layers.map((layer) => layer.name)).toEqual(["Layer 1"]);
    act(() => result.current.plateActions.addLayer());
    expect(result.current.plateState.layers.map((layer) => layer.name)).toEqual(["Layer 2", "Layer 1"]);
  ...
  test("preserves global selection and prunes all durable wells on shrink", () => {
    const { result } = renderHook(() =>
      usePlateReducer({
        ...
        initialLayers: [{
          id: ids[0],
          name: "Layer 1",
          annotations: [{ id: ids[1], label: "A", wells: [0, 95], annotationStyle: BLUE_STYLE }],
        }],
    ...
    expect(result.current.plateState.layers[0].annotations[0].wells).toEqual([0]);
  ...
  test("rejects invalid initialization and final-layer deletion", () => {
    expect(() => renderHook(() => usePlateReducer({ initialPlateSize: 24, initialLayers: [] }))).toThrow();
    const { result } = renderHook(() =>
      usePlateReducer({ initialPlateSize: 24, generateId: () => ids[0] }),
    );
    expect(() => act(() => result.current.plateActions.deleteLayer(ids[0]))).toThrow(
      "Cannot delete the final layer",
    );
  });
});
